...
- [x] **현장 QR 출력** — `/organizer/events/[eventId]/qr`: 심판·관람객 QR, A4 인쇄. QR에 password/session/secret **미포함**.
- [x] **관람객 QR/URL 검증** — `?tab=`·path route 모두 **관람 공개 기간** 서버 적용. live QR은 streaming ON + 공개 URL 필요. public DTO 민감정보 미포함.
