...
import {
  ...
  completeGymStaffAccountSetupAction,
...
export function GymStaffAccountSetupForm({
  token,
  staffName,
  gymName,
  existingLoginId,
  ...
  const [loginIdCheck, setLoginIdCheck] = useState<LoginIdCheckStatus>(
    existingLoginId ? "available" : "idle",
  );
  const [loginIdMessage, setLoginIdMessage] = useState<string | null>(
    existingLoginId
      ? "현재 아이디입니다. 변경하려면 중복확인을 다시 해 주세요."
      : null,
  );
  ...
  const loginIdReady =
    loginIdUnchanged ||
    (loginIdCheck === "available" && checkedLoginId === normalizedLogin);
  ...
  return (
    <form
      ...
      onSubmit={(e) => {
        ...
        void (async () => {
          try {
            const res = await completeGymStaffAccountSetupAction({
              token,
              loginId,
              password,
              passwordConfirm,
    ...
    >
      <section className="space-y-2">
        ...
        <dl className="space-y-1.5 text-sm text-matchon-text-secondary">
          <div className="flex flex-wrap gap-x-2">
            <dt className="font-medium text-matchon-text-primary">이름</dt>
            <dd>{staffName}</dd>
          </div>
          <div className="flex flex-wrap gap-x-2">
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-1.5">
          <label
            htmlFor="gym-staff-setup-login-id"
        ...
        <div className="space-y-1.5">
          <label
            htmlFor="gym-staff-setup-password"
            className="block text-xs font-medium"
          >
            비밀번호
          </label>
        ...
        <div className="space-y-1.5">
          <label
            htmlFor="gym-staff-setup-password-confirm"
            className="block text-xs font-medium"
          >
            비밀번호 확인
          </label>
          <div className="flex gap-2">
            <input
              id="gym-staff-setup-password-confirm"
