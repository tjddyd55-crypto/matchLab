...
import { GymStaffPasswordResetForm } from "@/components/domain/gym-staff/GymStaffPasswordResetForm";
...
export default async function GymStaffPasswordResetPage({
  params,
}: {
  params: Promise<{ token: string }>;
}) {
  const { token } = await params;
  const page = await gymStaffAccountSetupService.getResetPageByToken(token);
