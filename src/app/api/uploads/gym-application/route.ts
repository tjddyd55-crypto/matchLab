...
import { gymApplicationUploadService } from "@/lib/services/gym-application-upload.service";
  ...
  try {
    ...
    if (
      !Object.values(GymApplicationAttachmentType).includes(
        body.attachmentType as GymApplicationAttachmentType,
      )
    ) {
      return NextResponse.json(
    ...
    const result = await gymApplicationUploadService.issueUploadUrl({
      uploadBatchId: body.uploadBatchId,
      attachmentType: body.attachmentType as GymApplicationAttachmentType,
    ...
    console.error("[gym-application-upload]", e);
    return NextResponse.json(
