...
2. **`/login` → organizer** — `/organizer` 홈 → **내 대회** 상세 → 상단 **신청자** 메뉴로 `/organizer/events/{eventId}/applications` 이동 확인 → **현장·계체**(`/check-in`)에서 선수 검색·현장 확인·몸무게·계체 통과·출전 확정 단축 → 대진표·심판·경기 운영·결과 순. 심판 계정 3개 생성 → 경기 1개에 심판 배정 → `/judge/login`으로 채점·전송 → 운영 Drawer **심판 채점 집계** 확인 → **진행 시작 → 경기 종료 → 결과 입력** 후 공개 `/events/{slug}/results` 반영 확인. (선택: 행사 상세에서 **결과 입력 스태프 링크** 발급 후 시크릿 창에서 `/staff/result/…/matches` 확인.)
