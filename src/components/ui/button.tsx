...
/**
 * MATCHON Button SSOT — compact operational density.
 * default = 36px(md+) / 40px(mobile)
 * sm = 32px(md+) / 36px(mobile)
 * field = operational CTA alias of default (no h-11/h-12)
 * lg / icon-lg = public/hero opt-in only
 */
const buttonVariants = cva(
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-semibold whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      ...
      size: {
        default:
          "h-10 min-h-10 gap-1.5 px-4 md:h-9 md:min-h-9 has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3",
        ...
        sm: "h-9 min-h-9 gap-1 px-3 md:h-8 md:min-h-8 in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2 [&_svg:not([class*='size-'])]:size-3.5",
        /** public / marketing CTA opt-in */
        lg: "h-11 gap-1.5 px-4 text-[15px] has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3",
        ...
        "icon-lg": "size-11",
        /** alias of default — 업무형 주요 CTA (과거 h-11/12 제거) */
        field:
          "h-10 min-h-10 gap-1.5 px-4 md:h-9 md:min-h-9 has-data-[icon=inline-end]:pr-3 has-data-[icon=inline-start]:pl-3",
      },
    },
