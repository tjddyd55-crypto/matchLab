...
function ApplySummaryBlock({
  ...
  const matchedFighterCount = summary.matchedFighterCount ?? planned * 2;
...
export function AutoBracketGenerationPanel({
  ...
  const previewMatched =
    previewSummary?.matchedFighterCount ?? previewPlanned * 2;
  const previewUnmatched = previewSummary?.unmatchedCount ?? 0;
  const previewTotal =
    previewSummary?.totalFighterCount ?? previewMatched + previewUnmatched;
