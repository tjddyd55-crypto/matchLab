/**
 * Production QA — applicant excel sample structure v2 promotion.
 * Production yamabiko only. Never touches Preview/yamanote.
 *
 *   npx tsx scripts/e2e-applicant-excel-sample-structure-production-qa.mts
 */
...
const PREFIX = "PROD_APPLICANT_EXCEL_SAMPLE_QA_";
const BASE = "https://app-production-79ad.up.railway.app";
const OUT = join(
  process.cwd(),
  "test-results",
  "applicant-excel-sample-structure-production-qa",
);
...
async function main() {
  ...
  async function cleanup() {
    const events = await prisma.event.findMany({
      where: { publicSlug: { startsWith: "prod-sample-struct-qa-" } },
      select: { id: true },
    });
  ...
  try {
    ...
    const event = await prisma.event.create({
      data: {
        ...
        publicSlug: `prod-sample-struct-qa-${stamp}`,
