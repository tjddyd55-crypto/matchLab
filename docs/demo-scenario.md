...
1. **organizer** — `/organizer/events/{eventId}` 기본 설정 → **페이지 500 없이 로드**(`#setup-basic`) → **주소 검색**으로 도로명 선택 → 장소명·상세 주소 입력 → **기본 정보 저장** → 성공 메시지 → 새로고침 후 주소 유지. Railway 로그에 `.partial() cannot be used on object schemas containing refinements` **미발생** 확인.
...
6. **대회 관리** — 신청자 메뉴 정상 로드·EventManagementNav 표시. 관리 홈↔신청자↔현장·계체↔대진표 전환 시 콘텐츠 좌우 폭 일관성 확인.
7. **대진표 그룹** 상세 — compact VS 카드(높이 축소)·홍/청 대칭 스타일·하단 운영 정보(경기번호/매트/순서) 확인.
7. **spectator** `/events/sample-open-2026?tab=brackets` — 공개 VS 카드 홍/청 스타일이 관리 화면과 동일한지 확인.
8. **organizer** 현장·계체(`/check-in`) — 액션 버튼 가로 compact row·몸무게/메모 저장 피드백 확인 → 계체 실패 선수 대진 패 처리.
