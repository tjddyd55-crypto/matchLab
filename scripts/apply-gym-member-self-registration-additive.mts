/**
 * Apply GymMember self-registration additive schema.
...
if (!/yamanote/i.test(DATABASE_URL) || /yamabiko/i.test(DATABASE_URL)) {
  console.error("FAIL: expected Development yamanote DATABASE_URL");
  process.exit(1);
}

const sqlPath =
  process.env.SQL_PATH?.trim() ||
  "scripts/sql/add-gym-member-self-registration-additive.sql";
