// @ts-nocheck — upstream blossom.js uses dynamic locals; keep behavior identical.
/**
 * Edmonds blossom maximum-weight matching (general graphs).
 * Vendored from edmonds-blossom-fixed (MIT) — Matt Krick / Joris van Rantwijk.
 * Original: http://jorisvr.nl/maximummatching.html
 * @see ./edmonds-blossom.LICENSE.txt
 *
...
export function maxWeightMatching(
  ...
  // Sloppy-mode leaked locals from upstream blossom.js — keep as function-scoped vars.
  var b,
    v,
    w,
    path,
...
function Edmonds(this: any, edges: any, maxCardinality: any) {
  this.edges = edges;
...
function pIndex(arr, idx) {
  ...
  const edmonds = new (Edmonds as any)(edges, maxCardinality);
  return edmonds.maxWeightMatching() as number[];
}